...
5. writes PDF `DSS` / `VRI` structures on the final PDF and packages:
   - embedded certificate material
   - CRLs
   - raw OCSP responses when the responder is reachable
...
- the sidecar reports timestamp evidence in its LTV summary
...
- PDF `DSS` / `VRI` writing with cert/CRL/OCSP packaging
